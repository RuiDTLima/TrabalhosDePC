...
namespace TestServer {
     ...
     *  consola, devendo ser tomado em consideração que a escrita na consola pode aparecer fora de ordem.
     */
    class ServerClient {
                ...
                threads[i] = new Thread(() => {
                    foreach (string request in requests) {
                        using (TcpClient client = new TcpClient()) {
                            ...
                            Action<Tuple<string[], TcpClient>> apmServerClient = null;

                            if (!MESSAGE_HANDLERS.TryGetValue(cmd[0], out apmServerClient)) {
                                return;
                            }
                            // Dispatch request processing
                            apmServerClient(new Tuple<String[], TcpClient>(cmd, client));
                        }
                    }
                });
            }
